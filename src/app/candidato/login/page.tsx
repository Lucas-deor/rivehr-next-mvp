...
export default function CandidateLoginPage() {
  ...
  function handleSendOTP() {
    startTransition(async () => {
      const result = await candidateLoginAction(email.trim())
      if (result.success) {
  ...
  function handleVerifyOTP() {
    startTransition(async () => {
      const result = await verifyCandidateOTPAction(email.trim(), otp.trim())
      if (result.success) {
    ...
    <div className="flex items-center justify-center min-h-screen bg-muted/30 px-4">
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <p className="text-muted-foreground text-sm mt-1">Portal do Candidato</p>
        </div>

        <Card>
          <CardHeader>
            <CardTitle>
              {step === 'email' ? 'Entrar no Portal' : 'Verificar Código'}
            </CardTitle>
            <CardDescription>
              {step === 'email'
